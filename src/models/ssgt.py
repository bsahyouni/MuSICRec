...
#################################################
def build_subseq_graph_with_items(subseq_map, n_items, dist_threshold, dataset):
    """
    ...
    row_indices, col_indices, data_vals, ts_vals = [], [], [], []
    ...
    if os.path.exists(edge_file):
        ...
        ts_vals.extend([0.0] * len(cached["row"]))
    ...
    for s_id, items in subseq_map.items():
        for it in items:
            ...
            data_vals   += [1.0, 1.0]
            ts_vals += [dataset.ui_last_ts.get((s_id, it), 0.0)] * 2
    ...
    zero_cnt = sum(1 for t in ts_vals if t == 0)
    ts_vals = [t / SECS_PER_YEAR for t in ts_vals]
    edge_ts = torch.tensor(ts_vals, dtype=torch.float32)
    adj = torch.sparse.FloatTensor(i, v, torch.Size([size_total, size_total]))
    return adj, edge_ts
...
class SSGT(GeneralRecommender):
    ...
    def __init__(self,
                 ...
                 dataset,
                 data_extra):
        super(SSGT, self).__init__(config, dataset)
        ...
        self.dataset = data_extra
        self.uigraph_weight = 1.0  # weight factor for BPR loss
        self.register_buffer(
            "user_last_ts",
            torch.tensor(self.dataset.user_last_ts, dtype=torch.float32)
        )
        # ---------- TIME‑AWARE ADD‑ONS ----------------------------------
        self.delta = nn.Parameter(torch.tensor(1e-3))
        self.ss_convs = nn.ModuleList([
            LGConv() for _ in range(self.n_ss_layers)
        ])
        ...
        self.subseq_user = torch.arange(self.n_subseq, device=self.device)
        ...
        self.register_buffer("subseq_cache", torch.zeros(self.n_subseq, self.embedding_dim))
    ...
    def _refresh_subset(self, item_vec, subseq_ids):
        ...
        self.subseq_cache[subseq_ids] = self.att_pool(itm, msk)  # <<< EDIT
    ...
    def _refresh_all(self, item_vec):
        ...
        with torch.no_grad():  # avoid leaf-grad clash
            self.subseq_cache.copy_(seq_emb)  # <<< EDIT
    ...
    def get_subsequence_adj_mat(self):
        ...
        ss_item_adj, edge_ts = build_subseq_graph_with_items(
            subseq_map=self.subseq_map,
            n_items=self.n_items,
            dist_threshold=self.dist_threshold,
            dataset=self.dataset
        )
        self.register_buffer("edge_ts", edge_ts.to(self.device))
        return ss_item_adj

    #################################################
    # 3.3) FORWARD PASS
    #################################################
    def forward(self, ui_adj, batch_pos_items=None, batch_neg_items=None):
        """
        ...
        edge_idx = self.si_adj.indices()
        src_subseq = torch.where(edge_idx[0] < self.n_subseq, edge_idx[0], edge_idx[1])
        src_users = self.subseq_user[src_subseq]

        SECS_PER_YEAR = 31_536_000
        t_now = self.user_last_ts[src_users] / SECS_PER_YEAR

        age = (t_now - self.edge_ts)
        delta_pos = torch.clamp(F.softplus(self.delta), min=1e-5, max=0.2)
        w = torch.exp(-delta_pos * age)
        w = torch.clamp(w, min=1e-8, max=1.0)
        if torch.isnan(self.delta).any():
            print(">> DEBUG BREAK: δ became NaN")
            import pdb; pdb.set_trace()

        for conv in self.ss_convs:
            si_ego = conv(si_ego, edge_idx, w)

        # Step B3: split back out
        new_item_embed, new_subseq_embed = torch.split(
    ...
    def calculate_loss(self, interaction):
        ...
        ua_embeddings, ia_embeddings, sa_embeddings = self.forward(self.masked_adj, pos_items, neg_items)
...
class AdditiveAttentionPool(nn.Module):
    ...
    def __init__(self, d_model: int = 64, d_hidden: int = 32, max_len: int = 50):
        ...
        self.delta_att = nn.Parameter(torch.tensor(1e-3))
    ...
    def forward(self, item_emb: torch.Tensor, mask: torch.Tensor):
        ...
        # ---- attention ------------------------------------------------
        dt = torch.arange(L - 1, -1, -1, device=item_emb.device).float().unsqueeze(0).expand(B, -1)
        decay_rate = torch.clamp(F.softplus(self.delta_att), min=1e-5, max=0.2)
        decay = torch.exp(-decay_rate * dt)
        e = self.v_a(torch.tanh(self.W_a(x))).squeeze(-1) * decay
