...
class TrainDataLoader(AbstractDataLoader):
    ...
    def _get_neg_sample(self):
        ...
        if 'ssg' in self.config['model'].lower() or 'sgcl' in self.config['model'].lower():
            # seqs = [_random_prefix(self.dataset.get_user_seq(u))
            #         for u in u_ids]
            seqs = [self.dataset.get_user_seq(u) for u in u_ids]
            max_len = 50
            seq_items = np.zeros((len(seqs), max_len), dtype=np.int64)
            seq_mask = np.zeros_like(seq_items, dtype=np.float32)
            for i, s in enumerate(seqs):
                L = min(len(s), max_len)
                seq_items[i, :L] = s[-L:]
                seq_mask[i, :L] = 1.0
            seq_items = torch.from_numpy(seq_items).to(self.device)
            seq_mask = torch.from_numpy(seq_mask).to(self.device)
            return (batch_tensor, seq_items, seq_mask)
        else:
            return batch_tensor
...
class EvalDataLoader(AbstractDataLoader):
    ...
    def _next_batch_data(self):
        ...
        if self.config['model'].lower() == 'ssg' or self.config['model'].lower() == 'ssgt' or self.config['model'].lower() == 'sgcl':
            seqs = [self.dataset.get_user_seq(int(u)) for u in batch_users.cpu().numpy()]
            max_len = 50  # must match model.max_len
            seq_items = np.zeros((len(seqs), max_len), dtype=np.int64)
            seq_mask = np.zeros_like(seq_items, dtype=np.float32)
            for i, s in enumerate(seqs):
                L = min(len(s), max_len)
            ...
            seq_items = torch.from_numpy(seq_items).to(self.device)
            seq_mask = torch.from_numpy(seq_mask).to(self.device)
            return [batch_users, batch_mask_matrix, seq_items, seq_mask]
        else:
...
def _random_prefix(seq, min_len=2):
    """Return a prefix that always starts at index-0 and stops at
    a random position ∈ [min_len, len(seq)]."""
    if len(seq) <= min_len:
        return seq                     # nothing to crop
    stop = random.randint(min_len, len(seq))
    return seq[:stop]
