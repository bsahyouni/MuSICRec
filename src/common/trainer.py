...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        ...
        self.model_beta = config['model_beta']
        self.pooling_type = config['pooling_type']
        self.threshold = config['ss_dist_threshold']
        self.n_ss_layers = config['n_ss_layers']
        self.use_user_seq_fusion = config['use_user_seq_fusion']
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        running_loss = 0.0  # tensor on GPU
        #total_loss = None
        loss_batches = []
        bar = tqdm(train_data, desc=f"Epoch {epoch_idx+1}", leave=False)
        for batch_idx, interaction in enumerate(bar):
            ...
            # ---- regular / mirror‑gradient branch (unchanged) ----
            if self.mg and batch_idx % self.beta == 0:
                (self.alpha1 * loss).backward()
                self.optimizer.step()
                self.optimizer.zero_grad(set_to_none=True)

                loss2 = loss_func(interaction.clone())
                (-self.alpha2 * loss2).backward()
            else:
                loss.backward()
            ...
            running_loss += loss.detach()  # (b) no .item() per step
            loss_batches.append(loss.detach())

            # if batch_idx % 100 == 0:
            #     wandb.log({"batch_loss": loss.item()})
        epoch_loss = running_loss.mean().item()
        wandb.log({"epoch_loss": epoch_loss})
        return epoch_loss, loss_batches
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        wandb.init(project=f'{model_name}', name=f'{model_name}_{self.dataset_name}_{self.pooling_type}_{self.n_ss_layers}_{self.threshold}_{self.lambda_seqcl}_{self.lambda_itemcl}_{self.lambda_userseqcl}_{self.tau}')
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # wandb.log({"train_loss_epoch": train_loss_output})
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    ...
                    if saved:
                        ...
                        fn = f"{self.config['model']}-{self.config['dataset']}.pth"
                        try:
                            tag = self.config['checkpoint_name_tag']
                        except Exception:
                            tag = None
                        if tag:
                            fn = f"{fn}-{tag}"
                        ...
                        self.logger.info(f"✓ Saved best checkpoint to {path}")

                valid_log_dict = {}
                test_log_dict = {}
                for metric, val in valid_result.items():
                    valid_log_dict["valid_" + metric] = val
                for metric, val in test_result.items():
                    test_log_dict["test_" + metric] = val
                wandb.log({**valid_log_dict, **test_log_dict})
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        # batch full users
        batch_matrix_list = []
        for batched_data in tqdm(eval_data, desc="Evaluating", leave=False):
